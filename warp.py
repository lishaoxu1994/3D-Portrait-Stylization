...
def image_warp(img, point_pair, dst_pts, flow_pts, device, sharp=False, img2=None):
    ...
    #if sharp:
    w1, v1 = solve_interpolation(dst_pts - flow_pts, -flow_pts, device)
    flattened_flow_pair = apply_interpolation(point_pair, dst_pts - flow_pts, w1, v1, device)
    point_pair_ans = point_pair - flattened_flow_pair
    ...
    if img2 is None:
      return interpolated, warp_field, point_pair_ans
    else:
      return interpolated, interpolated2, warp_field, point_pair_ans
...
def solve_interpolation(train_points, train_values, device):
    ...
    #matrix_a += torch.randn_like(matrix_a)*1e-8
    #matrix_a_c = matrix_a.detach().cpu().clone().numpy()
    #matrix_a += 1e-4 * torch.eye(n).to(device)

    # Append ones to the feature values for the bias term in the linear model.
    ones = torch.ones([n, 1]).to(device)
    matrix_b = torch.cat([c, ones], 1)  # [n, d+1]
    #matrix_b += torch.randn_like(matrix_b)*1e-8
...
def apply_interpolation_pair(query_points, train_points, w, v, device):
    """Apply polyharmonic interpolation model to data.
...
def apply_warp(im, point_pair, src_list, dst_list, device, sharp=False, im2=None):
    ...
    for i in list(range(len(src_list)))[::-1]:
        ...
        if im2 is None:
          new_im, _, pair_warp = image_warp(new_im, point_pair, dst, dst-src, device, sharp=sharp)
        else:
          new_im, new_im2, warp_field, pair_warp = image_warp(new_im, point_pair, dst, dst-src, device, sharp=sharp, img2=new_im2)

    if im2 is None:
      return new_im, pair_warp
    else:
      return new_im, new_im2, warp_field, pair_warp.detach().cpu()
